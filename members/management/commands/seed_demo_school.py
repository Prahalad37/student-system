"""
Seed Demo School with 100 students and data for all ERP modules.
Usage: python manage.py seed_demo_school
"""
...
class Command(BaseCommand):
    help = "Seeds Demo School with 100 students and data for all modules"

    def handle(self, *args, **options):
        self.stdout.write(self.style.SUCCESS("Seeding Demo School...\n"))
        ...
        if created:
            self.stdout.write("Created school: Demo School")
        else:
            self.stdout.write("Using existing school: Demo School")

        # 2. Classes (DEMO-prefixed to avoid unique_together clash)
        self.stdout.write("Creating classes...")
        classes_data = [
            ("DEMO 6", "A"), ("DEMO 7", "A"), ("DEMO 8", "A"), ("DEMO 9", "A"), ("DEMO 10", "A"),
            ("DEMO 11", "Science"), ("DEMO 12", "Science"), ("DEMO 11", "Commerce"), ("DEMO 12", "Commerce"),
        ]
        classes = []
        ...
        for i in range(100):
            student = Member.objects.create(
                school=school,
                admission_no=f"DEMO2025{1000 + i}",
                firstname=random.choice(FIRST_NAMES),
                lastname=random.choice(LAST_NAMES),
                father_name=f"{random.choice(FIRST_NAMES)} {random.choice(LAST_NAMES)}",
                mobile_number=f"98765{53210 + (i % 100000)}",
                email=f"demo.student{i}@demoschool.edu",
                ...
                address=f"{random.randint(1, 150)} Education Lane, City",
        ...
        for fname, lname, designation in staff_data:
            st = Staff.objects.create(
                ...
                first_name=fname,
                last_name=lname,
                email=f"{fname.lower().replace(' ', '.')}.{lname.lower()}@demoschool.edu",
                phone=f"9876{random.randint(200000, 899999)}",
        ...
        for title, author, category in books_data:
            for _ in range(random.randint(2, 5)):
                book = Book.objects.create(
                    ...
                    title=title,
                    author=author,
                    isbn=f"ISBN-DEMO-{random.randint(1000000, 9999999)}",
                    category=category,
                    total_copies=1,
        ...
        routes_data = [
            ("Route A - City Center", "MH-12-DA-1234", "Rajesh Kumar", "9876543210"),
            ("Route B - East Zone", "MH-12-DB-5678", "Amit Sharma", "9876543211"),
            ("Route C - West Zone", "MH-12-DC-9012", "Vikram Singh", "9876543212"),
            ("Route D - North Zone", "MH-12-DD-3456", "Suresh Patel", "9876543213"),
        ]
        routes = []
        ...
        materials_data = [
            ("Algebra Basics", "Mathematics", "DEMO 8"),
            ("Light and Reflection", "Physics", "DEMO 10"),
            ("Organic Chemistry Intro", "Chemistry", "DEMO 11"),
            ("Essay Writing", "English", "DEMO 9"),
            ("Python Basics", "Computer", "DEMO 10"),
            ("Hindi Grammar", "Hindi", "DEMO 7"),
        ]
        ...
        for i in range(12):
            AdmissionEnquiry.objects.create(
                ...
                phone=f"98{random.randint(200000000, 899999999)}",
                email=f"enquiry{i}@demoschool.edu",
        ...
        if profile:
            for title, msg in [
                ("Welcome to Demo School ERP", "Your school data has been seeded successfully."),
